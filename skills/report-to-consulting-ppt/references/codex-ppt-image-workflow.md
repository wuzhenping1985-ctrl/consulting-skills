...
When this copied workflow is used inside `report-to-consulting-ppt`, the assembled image PPTX is an intermediate artifact unless the user explicitly asks for an image-only companion deck. The final user-facing PPTX for both report routes must be reconstructed through `pdf-to-editable-ppt` and must use the CIIC consulting template.
