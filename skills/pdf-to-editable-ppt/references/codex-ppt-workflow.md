...
When this copied workflow is used from `pdf-to-editable-ppt`, the normal final deliverable remains the template-based editable PPTX built on `assets/default-template.pptx`. The assembled image PPTX is allowed only as an intermediate artifact or as an explicitly requested image-only companion deck, not as a replacement for the CIIC consulting template route.
