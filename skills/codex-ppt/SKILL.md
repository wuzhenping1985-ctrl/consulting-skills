...
- The main agent owns orchestration, prompt jobs, dispatch/result recording, QA, speaker notes, and final PPT assembly. It should not silently replace slide subagents with its own sequential production when subagents are available.
- Every final `origin_image/slide_XX.png` must be generated by the selected image backend: built-in image generation/editing tool or `scripts/image_gen.py`. Local drawing, Pillow, SVG, HTML/CSS/canvas screenshots, python-pptx/PptxGenJS layouts, and manually composited text/image overlays are failure modes, not fallbacks.
...
- After sample approval, the main agent must record how the approved sample was generated and pass that exact method to every slide subagent.
...
### CIIC Consulting 2024 Color Specification

When the user asks for 中智咨询 style, CIIC consulting style, 新版 PPT 颜色规范, consulting report blue-white style, or provides the CIIC 2024 color reference image, use the following color system as a strict style constraint unless the user explicitly overrides it:

- Background: pure white, `RGB(255,255,255)`.
- Main text / body text: dark charcoal, `RGB(56,56,56)`. Do not use navy as the default body text color.
- Neutral surfaces, separators, and inactive modules: light gray `RGB(230,230,230)` and neutral gray around `RGB(220,220,220)`.
- Primary accent: vivid CIIC blue, close to `RGB(0,66,255)` / `#0042FF`. Use it for section titles, underline rules, arrows, icons, chart highlights, and key labels.
- Secondary accent blues: medium blue, light blue, and pale blue in the same hue family. Use them for chart series, hierarchy, and soft module backgrounds.
- Orange-red accent: close to `RGB(255,61,0)` / `#FF3D00`. Use only as tiny highlight marks, nodes, or emphasis tags; never use it as a large background area.
- Hyperlink / link-like accents should use the primary CIIC blue and neutral gray pairing.

Composition rules for this color system:

- Prefer white canvas, dark charcoal titles/body text, thin vivid-blue rule lines, light-gray cards, and blue line icons.
- Avoid large dark navy filled panels, heavy gradients, and saturated full-width headers.
- Avoid similar-tone pairings with weak contrast, especially blue text on blue backgrounds, white text on pale gray, or gray text on gray cards.
- Do not combine multiple blue fills at similar brightness in adjacent blocks unless there is a clear contrast boundary.
- Keep orange-red minimal and intentional; it should not compete with CIIC blue as the main brand color.
- For high-density consulting slides, use compact modules, dividers, arrows, matrix blocks, and small icons rather than decorative imagery.

### Management Consulting PPT Design Rules

Use these rules whenever the deck is for management consulting, strategy, organization, operating diagnosis, SOE reform, technology innovation planning, proposal, research, or executive briefing.
...
- Page system: use 16:9 widescreen. Content pages should be white-based; reserve full blue/visual backgrounds for cover, chapter, summary, or back-cover pages. Keep stable footer/logo/URL/page-marker placement when the style requires it.
...
- Action titles: write slide titles as conclusions, not topic labels. Prefer "科技企业需从单点技术突破转向全链条体系化创新" over "科技创新背景".
...
- Avoid: decorative gradients, heavy shadows, large dark navy panels on body pages, generic stock imagery, repeated same-card layouts on every slide, centered body paragraphs, text outside frames, duplicated text overlays, leftover placeholders such as `Sample`, and full slides made only of bullets.
