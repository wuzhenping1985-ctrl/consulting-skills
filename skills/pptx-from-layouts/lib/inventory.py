...
from pptx_compat import get_paragraph_element, get_presentation_from_slide, get_slide_layout_master
...
class ParagraphData:
    ...
    def __init__(self, paragraph: Any):
        ...
        _p = get_paragraph_element(paragraph)
        if (
            _p is not None
            and _p.pPr is not None
        ):
            pPr = _p.pPr
...
class ShapeData:
    ...
    def get_slide_dimensions(slide: Any) -> tuple[Optional[int], Optional[int]]:
        ...
        """
        try:
            prs = get_presentation_from_slide(slide)
    ...
    def _get_default_font_size(self) -> int:
        ...
        try:
            ...
            slide_master = get_slide_layout_master(self.shape)  # type: ignore
