...
def duplicate_slide(pres, index):
    ...
    image_rels = {}
    for rel_id, rel in six.iteritems(get_part_rels(source.part)):
        if "image" in rel.reltype or "media" in rel.reltype:
            image_rels[rel_id] = rel

    # CRITICAL: Clear placeholder shapes to avoid duplicates
    for shape in new_slide.shapes:
        sp = get_shape_element(shape)
        sp.getparent().remove(sp)

    # Copy all shapes from source
    for shape in source.shapes:
        el = get_shape_element(shape)
        new_el = deepcopy(el)
        insert_shape_element(new_slide, new_el)
        ...
        for blip in blips:
            ...
            if old_rId in image_rels:
                ...
                new_rId = get_or_add_rel(
                    get_part_rels(new_slide.part), old_rel.reltype, get_rel_target(old_rel)
                )
    ...
    for rel_id, rel in image_rels.items():
        try:
            get_or_add_rel(get_part_rels(new_slide.part), rel.reltype, get_rel_target(rel))
        except Exception:
...
def delete_slide(pres, index):
    ...
    sldIdLst = get_slide_id_list(pres)
    rId = sldIdLst[index].rId
    drop_rel(pres.part, rId)
    del sldIdLst[index]


def reorder_slides(pres, slide_index, target_index):
    """Move a slide from one position to another."""
    slides = get_slide_id_list(pres)
