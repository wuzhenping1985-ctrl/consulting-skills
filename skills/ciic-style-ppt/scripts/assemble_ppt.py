...
def remove_all_template_slides(prs) -> None:
    """Keep template masters, layouts, and theme, but remove example slides."""
...
def get_blank_slide_layout(prs):
    """Return a blank-like layout without assuming every template has index 6."""
...
def create_presentation(
    ...
    try:
        ...
        if template_path:
            ...
            remove_all_template_slides(prs)
            print(f"Using PPT template: {template_path}")
        else:
            prs = Presentation()

        # 设置幻灯片尺寸；模板模式下保留模板原始页面尺寸。
        if template_path:
            print(f"Template slide size preserved: {prs.slide_width} x {prs.slide_height} EMU")
        ...
        for i, image_path in enumerate(image_files, 1):
            ...
            # 使用空白布局（索引 6）
            blank_slide_layout = get_blank_slide_layout(prs)
            ...
            print(f"[OK] 已添加第 {i} 页: {os.path.basename(image_path)}")
...
注意：
  ...
  - 如果项目目录下存在 speech.md，会按 Slide N 标题写入每页备注
    ...
    parser.add_argument('--template',
                        default=os.environ.get('CODEX_PPT_TEMPLATE'),
                        help='Optional PPTX template path. Preserves template masters, layouts, and theme.')
    ...
    if args.init:
        ...
        print(f"[OK] PPT 项目目录已准备好: {ppt_project_dir}")
        print(f"[OK] 幻灯片图片目录已准备好: {origin_image_dir}")
        sys.exit(0)
